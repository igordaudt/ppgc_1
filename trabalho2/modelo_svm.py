# modelo_svm.py
# Classificação de materiais de construção usando Support Vector Machine (SVM)
...
MODEL_PKL   = MODEL_DIR / "svm_model.pkl"
...
def build_model():
    """Cria o modelo SVM linear (LinearSVC) adequado para dados TF-IDF de alta dimensionalidade."""
    model = LinearSVC(
        C=1.0,             # parâmetro de regularização
        max_iter=2000,     # garante convergência
        class_weight=None, # pode ajustar para balanceamento, se necessário
        random_state=42
    )
    return model
